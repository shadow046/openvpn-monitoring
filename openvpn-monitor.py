...
class OpenvpnHtmlPrinter(object):
    ...
    def print_html_header(self):
        ...
        output('<a class="dropdown-toggle" data-toggle="dropdown" href="#">Pipes')
    ...
    def print_session_table_headers(vpn_mode, show_disconnect):
        server_headers = ['Username / Hostname', 'UPLOAD',
                          'DOWNLOAD', 'Connected Since', 'Last Ping', 'Time Online']
        if show_disconnect:
    ...
    def print_vpn(self, vpn_id, vpn):
        ...
        output('<thead><tr><th>Status</th><th>Pingable</th>')
        output('<th>Clients</th><th>Total Upload</th><th>Total Download</th>')
        output('<th>Up Since</th>')
        if vpn_mode == 'Client':
            output('<th>Remote IP Address</th>')
        output('</tr></thead><tbody>')
        output('<tr><td>{0!s}</td>'.format(connection))
